...
/**
 * @description Displays and controls playback for one audio element.
...
export function AudioElementCard({ element, mode }: AudioElementCardProps) {
  ...
  const handleDelete = async () => {
    try {
      ...
      error(`Failed to remove ${mode === 'one-shot' ? 'one-shot' : 'element'}`);
    }
  };
      ...
      <div className="flex-1 flex flex-col justify-center min-w-0 z-10">
        ...
        <div className="flex items-center gap-2 mt-1 relative">
          ...
          <button
            ...
            title={`Remove ${mode === 'one-shot' ? 'One-shot' : 'Element'}`}
          >
