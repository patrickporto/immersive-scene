        ...
        <Stack className="flex-1 items-center overflow-y-auto no-scrollbar" gap={3}>
          {soundSets.map(soundSet => (
            <motion.div
              ...
              className={cn(
                'w-10 h-10 rounded-xl flex items-center justify-center cursor-pointer transition-all border relative',
            ...
            >
              ...
              {selectedSoundSetIds.includes(soundSet.id) &&
                selectedSoundSet?.id !== soundSet.id && (
                  <div className="absolute top-1 right-1 w-1.5 h-1.5 rounded-full bg-cyan-400 shadow-[0_0_5px_rgba(0,212,255,1)]" />
                )}
            </motion.div>
          ))}
        </Stack>
  ...
  return (
    <Stack className="h-full bg-[#0f0f15]" gap={0}>
      ...
      <div className="flex-1 overflow-hidden flex flex-col">
          ...
          <Stack className="overflow-y-auto custom-scrollbar" gap={2}>
            ...
            {soundSets.map(soundSet => (
              <SoundSetListItem
                ...
                id={soundSet.id}
                name={soundSet.name}
                isEnabled={soundSet.is_enabled}
                isSelected={selectedSoundSet?.id === soundSet.id}
                isViewSelected={selectedSoundSetIds.includes(soundSet.id)}
        ...
        <div className="border-t border-white/10 bg-black/40 flex-1 flex flex-col min-h-0">
          <Cluster
            className="px-4 py-3 border-b border-white/5 bg-black/20"
            justify="between"
            align="center"
