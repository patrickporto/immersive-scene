  ...
  return (
    <div className="pt-6 border-t border-white/5 bg-black/20 p-6 lg:p-8 transition-colors">
      ...
      <div className="grid grid-cols-[repeat(auto-fill,minmax(300px,1fr))] gap-3">
        ...
        <Droppable droppableId="global-one-shots" direction="horizontal" isCombineEnabled={true}>
          {(provided, snapshot) => (
            <div
              className={cn(
                'contents p-2 rounded-xl transition-all border-2 border-transparent block',
                snapshot.isDraggingOver &&
                  'border-fuchsia-500/50 bg-fuchsia-500/5 shadow-[inset_0_0_20px_rgba(232,121,249,0.1)]'
              )}
            ...
            >
              ...
              <AnimatePresence mode="popLayout">
                {globalOneShots.map((element, index) => (
                  <Draggable
                    key={element.id.toString()}
                    draggableId={element.id.toString()}
                    index={index + groups.filter(g => g.sound_set_id === null).length}
                  >
                    {(dragProvided, dragSnapshot) => (
                      <div
                        ref={dragProvided.innerRef}
